...
export const ManageSectionModal = ({ isOpen, onClose, onSave, section = null, category }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md border border-slate-200 dark:border-slate-800 shadow-2xl">
        <h2 className="text-lg font-bold mb-4 dark:text-white">{section ? 'Edit Section' : 'Create Section'} in {category.toUpperCase()}</h2>
